...
/*   Updated: 2026/07/28 11:18:29 by vaugusto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	funnel_err(char identifier, va_list args)
{
	if (identifier == 'c')
		return (ft_print_2c(va_arg(args, int)));
	else if (identifier == 's')
		return (ft_print_2s(va_arg(args, char *)));
	else if (identifier == 'd')
		return (ft_print_2i(va_arg(args, int)));
...
int	ft_printf_err(const char *format, ...)
{
	...
	{
		if (format[i] == '%')
			total += funnel_err(format[++i], args);
		else
			total += ft_print_2c(format[i]);
		i++;
	}
	va_end(args);
	return (total);
